...
\noindent Correctness:
\[
\forall AState; CState; CState'; CInputs? @ \pre AOp \land R \land COp \implies \exists AState'; AInputs? @ R' \land AOp
\]
...
\exists sacc : \seq[Transaction] @\\
      \exists d : Date; desc : DESCRIPTION; a : ACCOUNT; i  : MONEY; e : MONEY) @\\
           (d= date? \land\\
            ...
            sacc = saccount \cat <  >)
\end{schema}
\noindent and by using the one-point rule on all the existential quantifications here we can see that the predicate part is just $true$. So, and state and inputs (of the corrects types) are states we can start from, do the $DoSomeBusiness$ operation and be sure of ending in a valid state (i.e. the precondition of the operation is true). Applicability is then the trivial:
\[
\forall SingleState; DoubleState @ true \land R \implies true
\]

Finally correctness. As we same, the precondition for $DoSomeBusiness$ is true so correctness in this case is the slightly simpler statement
\[
\forall SingleState; DoubleState; DoubleState' @ R \land DoSomeDoubleBusiness \implies\\
\t2  \exists SingleState' @ R' \land DoSomeBusiness
\]
...
and $DoSomeDoubleBusiness$ similarly has the simpler equivalent definition
...
\[
\forall saccount : \seq Transaction; 
daccount, daccount' :  \seq DoubleTransaction; \\
...
( \t1 saccount'(i).date = daccount'(i).ddate \land\\
       ...
       \t1 saccount'(i).income = daccount'(i).debit2 \land\\
       \t1 saccount'(i).expenditure = daccount'(i).credit1 \land\\
       \t1 saccount' = saccount \cat < \lbind date \mapsto date?, description \mapsto description?, \\
\t2 account \mapsto account?, income \mapsto income?, expenditure \mapsto expenditure?  \rbind >
\]

The antecedent tells us that the two sequences $saccount'$ and $daccount'$ have the relationship required in the consequent for all elements their beginning up to the length of $saccount$. Also, given that the lengths of the two sequences $saccount$ and $daccount$ are given as equal in the antecedent, and given that $saccount'$ and $daccount'$ are both just one elements longer than $saccount$ and $daccount$, it follows that $saccount'$ and $daccount'$ have the same length. Then all that is left to prove is that the two new elements added to the two sequences have the correct relationship, i.e. that
\[
...
\t3 account? = account1? = account2? \land \\
\t3 income? = debit2? \land expenditure? = credit1?
\]

and this follows from several uses of the one-point rule.
